...
func getUdAttr(attr string, n *alpinods.Ud) string {
	switch attr {
	case "id":
		return n.ID
	case "form":
...
func getDepAttr(attr string, n *alpinods.Dep) string {
	switch attr {
	case "id":
		return n.ID
	case "head":
...
func conllu2svg(fp io.Writer, id int, alpino *alpinods.AlpinoDS, ctx *TreeContext, data []byte) {

	if alpino.Conllu == nil || alpino.Conllu.Status == "" {
		conllu, err := alud.Ud(data, "", "", alud.OPT_NO_COMMENTS|alud.OPT_NO_DETOKENIZE)
		if err == nil {
			alpino.Conllu = &alpinods.Conllu{Status: "OK", Conllu: conllu}
		} else {
			e := err.Error()
			i := strings.Index(e, "\n")
			if i > 0 {
			...
			alpino.Conllu = &alpinods.Conllu{Status: "error", Error: e}
		}
	}
	...
	fmt.Fprint(fp, `<script type="text/javascript">
`)
	fmt.Fprint(fp, jquery)
	fmt.Fprint(fp, `
var tts = [];
var normal = [];
